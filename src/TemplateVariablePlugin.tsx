...
export class TemplateVariableNode extends TextNode {
  ...
  createDOM<EditorContext extends Record<string, any>>(
    ...
    element.classList.add('templateVariable');
    if (!this.isValidVariableName()) {
      element.classList.add('invalid');
    }
    return element;
...
function getTemplateVariableRegexStringChars(): Readonly<{
  alpha: string;
  alphanumeric: string;
  ...
  // underscore. These characters can be included in templateVariables, but a templateVariable
  ...
  const variableChars = '#\\uFF03'; // normal '#' or full-width '#'
...
function getTemplateVariableRegexString(): string {
  const { alpha, alphanumeric, variableChars } =
    getTemplateVariableRegexStringChars();

  const templateVariableAlpha = '[' + alpha + ']';
  const templateVariableAlphanumeric = '[' + alphanumeric + ']';
  const templateVariableBoundary = '^|$|[^&/' + alphanumeric + ']';
  const variableCharList = '[' + variableChars + ']';

  // A templateVariable contains characters, numbers and underscores,
  // but not all numbers.
  const templateVariable =
    '(' +
    templateVariableBoundary +
    ')(' +
    variableCharList +
    ')(' +
    templateVariableAlphanumeric +
    '*' +
    templateVariableAlpha +
    templateVariableAlphanumeric +
    '*)';
...
const REGEX = new RegExp(getTemplateVariableRegexString(), 'i');
...
export function TemplateVariablePlugin({
  ...
  const getTemplateVariableMatch = useCallback((text: string) => {
    ...
    const templateVariableLength = matchArr[3]!.length + 1;
    const startOffset = matchArr.index + matchArr[1]!.length;
    const endOffset = startOffset + templateVariableLength;
    return { end: endOffset, start: startOffset };
  }, []);

  const createTemplateVariableNode = useCallback(
    (textNode: TextNode): TemplateVariableNode => {
